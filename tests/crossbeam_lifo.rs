//! LIFO work-stealing deque tests **ported from `crossbeam-deque`**
//! (`crossbeam-deque/tests/lifo.rs`), adapted to `ws-deque`'s API:
//! `Worker::new_lifo()` → `Worker::new()`, crossbeam's `scope` → `std::thread::scope`.
//!
